...
function parseManifestJson(manifestJson) {
  if (typeof manifestJson !== 'string' || Buffer.byteLength(manifestJson, 'utf8') > MAX_INLINE_BYTES) throw new RecoveryProofError('INVALID_INPUT', 'manifestJson must be a string no larger than 1 MiB')
  let value
  try { value = JSON.parse(manifestJson) } catch { throw new RecoveryProofError('INVALID_JSON', 'manifestJson is not valid JSON') }
  return { manifest: validateManifest(value), manifestSha256: sha256(manifestJson) }
}
...
async function loadEvents(workspaceRoot, eventsPath, manifest) {
  ...
  return parseEventsJsonl(raw, manifest)
}

function parseEventsJsonl(raw, manifest) {
  if (typeof raw !== 'string' || Buffer.byteLength(raw, 'utf8') > MAX_INLINE_BYTES) throw new RecoveryProofError('INVALID_INPUT', 'eventsJsonl must be a string no larger than 1 MiB')
...
async function writeReport(workspaceRoot, artifactDir, report) {
  ...
  try {
    ...
    return { path: path.relative(await realpath(workspaceRoot), target).replaceAll(path.sep, '/'), sha256: digest, bytes: Buffer.byteLength(reread), verifiedByReadBack: true }
  } finally {
...
export function inspectRecoveryManifestJson(manifestJson) {
  const { manifest, manifestSha256 } = parseManifestJson(manifestJson)
  return {
    schemaVersion: 1,
    operation: 'inspect-inline-manifest',
    proofOnly: true,
...
export function verifyRecoveryEvidenceJsonl(manifestJson, eventsJsonl) {
  const { manifest, manifestSha256 } = parseManifestJson(manifestJson)
  const { events, eventsSha256 } = parseEventsJsonl(eventsJsonl, manifest)
  const scenarios = verifyScenarios(manifest, events)
  return {
    schemaVersion: 1,
    operation: 'verify-inline-evidence',
    proofOnly: true,
    filesystemAccess: false,
    objectContentVerification: 'not-performed',
    inputs: { manifestSha256, eventsSha256 },
    eventCount: events.length,
    status: scenarios.every(({ status }) => status === 'verified') ? 'structurally-verified' : 'failed',
    disclosure: { objectHashesDeclaredButNotDereferenced: manifest.objects.map(({ id }) => id) },
    scenarios
  }
}
