...
const child = spawn(nodeExecutable, [resolve(checkout, 'apps/cli/lib/bin.js'), '--profile', 'web', '--host', '127.0.0.1', '--port', '0'], {
  cwd: resolve(checkout), env: safeEnv, shell: false, stdio: ['ignore', 'pipe', 'pipe'], windowsHide: true
})
let stdout = ''
let stderr = ''
